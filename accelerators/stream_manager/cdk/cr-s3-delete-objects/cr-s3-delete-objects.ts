...
export class CustomResourceS3DeleteObjects extends cdk.Construct {
    ...
    constructor(scope: cdk.Construct, id: string, props: CustomResourceS3DeleteObjectsProps) {
        super(scope, id);
        ...
        new cfn.CustomResource(this, 'Resource', {
            provider: cfn.CustomResourceProvider.fromLambda(new lambda.SingletonFunction(this, 'Singleton', {
                ...
                code: lambda.Code.fromAsset('cr-s3-delete-objects/cr_s3_delete_objects'),
                handler: 'index.main',
                timeout: cdk.Duration.seconds(30),
                runtime: lambda.Runtime.PYTHON_3_8,
                initialPolicy: [
                    new iam.PolicyStatement({
                        actions: ['s3:ListBucket', 's3:ListBucketMultipartUploads', 's3:ListBucketVersions', 's3:DeleteBucket'
                        ],
                        resources: ['arn:aws:s3:::' + props.bucketName]
                    }),
                    new iam.PolicyStatement({
                        actions: ['s3:List*', 's3:DeleteObject', 's3:DeleteObjectVersion',
                        ],
                        resources: ['arn:aws:s3:::' + props.bucketName + '/*']
                    }),
                ]
